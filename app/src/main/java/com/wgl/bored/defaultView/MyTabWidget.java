...
public class MyTabWidget extends LinearLayout {
    ...
    //底部菜单的图标数组
    private int[] mDrawableIds = new int[]{R.drawable.main_tab_bg_1,
            R.drawable.main_tab_bg_2, R.drawable.main_tab_bg_3,
            R.drawable.main_tab_bg_4};
    ...
    public MyTabWidget(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (null == mLabels || mLabels.length <= 0) {
            Toast.makeText(context, "底部菜单的文字数组未添加...", Toast.LENGTH_SHORT).show();
            a.recycle();
            return;
        }
        if(null == mDrawableIds || mDrawableIds.length <= 0){
            Toast.makeText(context, "底部菜单的图标数组未添加...", Toast.LENGTH_SHORT).show();
            a.recycle();
            return;
        }
...
//                checkedTextView.setTextColor(Color.rgb(247, 88, 123));
                checkedTextView.setTextColor(Color.parseColor("#FFB400"));
                mViewList.get(i).setBackgroundColor(Color.parseColor("#FFFFFF"));
            } else {
                checkedTextView.setChecked(false);
//                checkedTextView.setTextColor(Color.rgb(19, 12, 14));
                checkedTextView.setTextColor(Color.parseColor("#0F85E0"));
                mViewList.get(i).setBackgroundColor(Color.parseColor("#FFFFFF"));
            }
        }
    }
